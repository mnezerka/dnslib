
#include <iostream>

#include "message.h"
#include "rr.h"
#include "buffer.h"
#include "assert.h"

using namespace std;

void testBuffer()
{
    // check character string
    char packet1[] = "\x05helloworld";
    dns::Buffer b(packet1, sizeof(packet1) - 1);
    std::string strHello = b.getDnsCharacterString();
    assert (strHello == "hello");

    // check domain name
    //char packet2[] = "";
}

void testNAPTR()
{
    dns::RDataNAPTR r;

    char naptr1[] = "\x00\x32\x00\x33\x01\x73\x07\x53\x49\x50\x2b\x44\x32\x54\x00\x04\x5f\x73\x69\x70\x04\x5f\x74\x63\x70\x05\x69\x63\x73\x63\x66\x05\x62\x72\x6e\x35\x36\x03\x69\x69\x74\x03\x69\x6d\x73\x00";
    dns::Buffer b(naptr1, sizeof(naptr1) - 1);
    cout << "decoding naptr of length: " << sizeof(naptr1) - 1 << " bytes" << endl;

    r.decode(b);
}

void testPacket()
{
    //
    //                 00                                      10                                      20                                      30      32          35                  40
    char packet[] = "";
    //                                                                  3   w   w   w   6   g   o   o   g   l   e   3   c   o   m   0   t   t   c   c  link      type   class             ttl rlength -----------------rdata
    //               ---------------------------------------header--|--------------------------------------- query1 --------------------------------|-------------------- answer1 ----------------------------------------------------------------------------------

    cout << "decoding packet of length: " << sizeof(packet) << " bytes" << endl;
    dns::Message q;
    q.decode(packet, sizeof(packet) - 1);
    cout << q.asString() << endl;

    char packet1[] = "\x5a\x2e\x01\x20\x00\x01\x00\x00\x00\x00\x00\x01\x04\x61\x68\x6f\x6a\x00\x00\x01\x00\x01\x00\x00\x29\x10\x00\x00\x00\x00\x00\x00\x00";
    cout << "decoding packet of length: " << sizeof(packet1) << " bytes" << endl;
    dns::Message m1;
    m1.decode(packet1, sizeof(packet1) - 1);
    cout << m1.asString() << endl;

    char packet2[] = "";
    cout << "decoding packet of length: " << sizeof(packet2) << " bytes" << endl;
    dns::Message m2;
    m2.decode(packet2, sizeof(packet2) - 1);
    cout << m2.asString() << endl;
}

void testCreatePacket()
{
    char packet2[] = "";
    //char packet2[] = "\x06\x1c\x01\x20\x00\x01\x00\x00\x00\x00\x00\x01\x05\x62\x72\x6e\x35\x36\x03\x69\x69\x74\x03\x69\x6d\x73\x00\x00\x23\x00\x01\x00\x00\x29\x10\x00\x00\x00\x00\x00\x00\x00";
    dns::Message answer;
    answer.decode(packet2, sizeof(packet2) - 1);
    cout << answer.asString() << endl;

    char outputBuffer[2000];
    dns::uint outputBufferSize;
    answer.encode(outputBuffer, 2000, outputBufferSize);
}

void testMemoryAllocation()
{
    char packet[] = "\x06\x1c\x01\x20\x00\x01\x00\x00\x00\x00\x00\x01\x05\x62\x72\x6e\x35\x36\x03\x69\x69\x74\x03\x69\x6d\x73\x00\x00\x23\x00\x01\x00\x00\x29\x10\x00\x00\x00\x00\x00\x00\x00";

    dns::Message answer;
    answer.decode(packet, sizeof(packet) - 1);
    answer.setQR(dns::Message::typeResponse);

    // add NAPTR answer
    dns::ResourceRecord *rr = new dns::ResourceRecord();
    rr->setType(dns::ResourceRecord::typeNAPTR);
    rr->setClass(dns::ResourceRecord::ClassIN);
    rr->setTtl(60);
    dns::RDataNAPTR *rdata = new dns::RDataNAPTR();
    rdata->setOrder(50);
    rdata->setPreference(51);
    rdata->setServices("SIP+D2T");
    rdata->setRegExp("");
    rdata->setReplacement("_sip._tcp.icscf.brn56.iit.ims");
    rr->setRData(rdata);

    answer.addAnswer(rr);

    dns::uint mesgSize;
    char mesg[2000];
    answer.encode(mesg, 2000, mesgSize);
}

int main(int argc, char** argv)
{
    //testBuffer();
    //testNAPTR();
    //testPacket();
    testCreatePacket();
    testMemoryAllocation();

    return 0;
}
